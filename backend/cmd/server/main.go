...
func main() {
	...
	measurement := repository.NewMeasurementRepository(db)
	hub := ws.NewHub()
	...
	measurementService := service.NewMeasurementService(measurement, greenhouses, sensors, l, hub)
	engine := router.New(router.Dependencies{Config: cfg, Logger: l, Auth: auth, Monitoring: monitoring, Alerts: alertService, Control: control, Reports: reports, Measurement: measurementService, Hub: hub})
